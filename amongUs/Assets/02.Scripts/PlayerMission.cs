...
public class PlayerMission : MonoBehaviour
{
    ...
    public void DisplayText()
    {
        ...
        {
            displayText.text += myMission[i].GetComponent<MissionData>().MissionPlace + " 에서 " +
                missionManager.GetInformation(myMission[i].GetComponent<MissionData>().MissionType,
    ...
    public void createMission()
    {
        ...
        for (int i = 0; i < HaveMission.Length; i++) //설정대로 미션을 돌려줌 
        {
            ...
            if (HaveMission[x].GetComponent<MissionData>().MissionType == MissionManager.MissionCommon && missionManager.commonMissionNum > 0)
            {
                ...
                missionManager.commonMissionNum--;
            }
            //x번째 HavaMission의 미션타입과 미션매니저의 단순미션이 같고, 미션매니저의 단순임무 갯수가 0보다 크다면
            else if (HaveMission[x].GetComponent<MissionData>().MissionType == MissionManager.MissionSimple && missionManager.simpleMissionNum > 0)
            {

                //myMission에 x번째 HavaMission을 추가
                //미션매니저의 단순임무 갯수를 줄인다.
                myMission.Add(HaveMission[x]);
                missionManager.simpleMissionNum--;
            }
            //x번째 HavaMission의 미션타입과 미션매니저의 복잡미션이 같고, 미션매니저의 복잡임무 갯수가 0보다 크다면
            else if (HaveMission[x].GetComponent<MissionData>().MissionType == MissionManager.MissionDifficult && missionManager.difficultMissionNum > 0)
            {
                //myMission에 x번째 HavaMission을 추가
                //미션매니저의 복잡임무 갯수를 줄인다.
                myMission.Add(HaveMission[x]);
                missionManager.difficultMissionNum--;
            }
        }
    }
